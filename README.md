...
To generate samples after training, run:

```bash
uv run -m ddpm.sample --checkpoint-path=<path_to_checkpoint>
```

The following figure shows an example of samples generated by a model trained on the CelebA dataset at 64x64 resolution for 200 epochs.

![DDPM reverse](./img/ddpm_reverse.gif)
![DDPM Progress](./img/ddpm_reverse.png)
